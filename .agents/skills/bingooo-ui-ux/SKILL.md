---
name: bingooo-ui-ux
description: Canonical design and implementation skill for every Bingooo customer-facing and admin page. Use when building, styling, or updating any Bingooo page, component, header, footer, product card, or admin view.
---

# Bingooo UI/UX Skill --- Single Source of Truth

> This file is the canonical design and implementation skill for every
> Bingooo customer-facing and admin page. Future work must extend this
> system instead of inventing a new visual language.

## AI / Design-Agent Operating Rule

Before designing or coding any Bingooo page, read this file and treat it
as the source of truth. Reuse existing components and tokens first.
Preserve the same typography, palette, spacing, radius, borders,
shadows, iconography, header, footer, forms, product cards, and
responsive behavior. A new page may introduce a new content layout, but
it must not introduce a new design language.

**Primary typeface is locked to Manrope.**

**Brand palette is locked to Bingooo red `#E6321C`, deep red `#B91F12`,
warm cream `#F7EEDB`, soft beige `#EDE0CC`, charcoal `#171717`, white
`#FFFFFF`, muted text `#6F6A63`, and border `#DDD3C5`.**

**Visual quality target:** premium contemporary menswear, warm
minimalism, editorial photography, strong typography, generous
whitespace, restrained UI, and no obvious AI-slop aesthetics.

## Non-Negotiable Consistency Checklist

-   Use Manrope across storefront and admin.
-   Use the supplied Bingooo logo without distortion, recoloring,
    redrawing, or stretching.
-   Use the existing Bingooo color tokens; do not introduce arbitrary
    accent colors.
-   Reuse shared UI components before creating new ones.
-   Keep red as an intentional action/accent color rather than a
    universal background.
-   Keep product imagery realistic, premium, and fashion-editorial.
-   Maintain the same spacing scale, radius system, border treatment,
    icon style, and interaction language.
-   Every new page must be responsive at mobile, tablet, and desktop
    widths.
-   Include appropriate loading, empty, error, success, disabled, hover,
    focus, and active states.
-   Do not create page-specific styling that visually conflicts with the
    rest of Bingooo.
...
Primary typeface: **Manrope**. Fallback: system-ui, sans-serif.
...
Visual: - all pages use Bingooo palette - all pages use Manrope - shared
...
**Warm cream + beige + charcoal + Bingooo red + Manrope + editorial
...
For every new page, follow this order:

1.  **Read this skill completely.**
2.  **Identify the page goal** and the primary user action.
3.  **Reuse existing components** and extend them only when necessary.
4.  **Use design tokens**, never random hard-coded styling.
5.  **Build the information hierarchy first**: title → context → primary
    action → content.
6.  **Match existing Bingooo page density and visual rhythm.**
7.  **Design mobile intentionally**, not as a shrunken desktop version.
8.  **Implement all meaningful UI states.**
9.  **Run an accessibility pass.**
10. **Compare the result against the checkout page, product page, shop
    page, and admin visual language before finalizing.**

### New Component Rule

If a new component is required, it must look like a native Bingooo
component. Do not create isolated variants such as `ProductCardNew`,
`CheckoutButton2`, or unrelated page-specific styling unless the
information architecture genuinely requires a distinct semantic
component.

### Do / Don't

**Do:** use whitespace, typography, product imagery, subtle borders,
restrained shadows, warm surfaces, and clear hierarchy.

**Don't:** use random gradients, neon colors, purple/blue SaaS styling,
glassmorphism, excessive pills, giant shadows, excessive animation,
decorative clutter, or generic AI-generated fashion imagery.

## 79. Definition of Done

A Bingooo page is complete only when it is visually consistent,
responsive, accessible, usable, technically integrated with the project
architecture, and visually indistinguishable from the same product
