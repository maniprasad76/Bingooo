...
export function AnalyticsPage() {
  ...
  const { data: analytics, isLoading } = useQuery({
    ...
    queryFn: () => api.get<any>('/admin/analytics', { timeRange }),
  });

  const grossSales = analytics?.grossSales ?? 508135;
  const aov = analytics?.aov ?? 1435;
  const customStudioRate = analytics?.customStudioRate ?? '18.2%';
  const customerRepeatRate = analytics?.customerRepeatRate ?? '34.8%';
  const topProducts = analytics?.topProducts ?? [];
  const categoryBreakdown = analytics?.categoryBreakdown ?? [];
  const salesVelocityBars = analytics?.salesVelocityBars ?? [];
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="flex min-h-[300px] items-center justify-center gap-3 text-muted">
          ...
          <span className="text-xs font-bold uppercase tracking-wider">Aggregating telemetry...</span>
        </div>
      ) : (
        <>
          {/* KPI Cards Grid */}
