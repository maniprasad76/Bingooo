        ...
        <div className="group relative flex h-full flex-col rounded-2xl bg-white border border-border/80 overflow-hidden shadow-card transition-all duration-500 hover:shadow-card-hover hover:border-ink/25">
          ...
          <div className="relative aspect-[4/5] w-full bg-[#EDE0CC] flex items-center justify-center p-4 overflow-hidden">
            ...
            <div className="absolute top-3.5 left-3.5 flex flex-col gap-1.5 z-10">
              {customizationEnabled && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#E6321C] text-white text-[10px] font-sans font-bold uppercase tracking-wider shadow-sm">
                  <Sparkles size={11} />
                  Custom Studio
                </span>
              )}
              {discountPct && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-[#171717] text-white text-[10px] font-sans font-bold tracking-wider shadow-sm">
                  {discountPct}% OFF
                </span>
              )}
            </div>
          ...
          <div className="p-5 flex flex-1 flex-col justify-between bg-white">
            <div>
              <div className="flex items-center justify-between mb-1.5">
                {category && (
                  <span className="text-[10px] font-sans font-bold uppercase tracking-wider text-[#6F6A63]">
                    {category.name}
                  </span>
                )}
                <span className="text-[10px] font-sans font-semibold text-[#6F6A63]/80">
                  220 GSM
                </span>
              </div>

              <Link to={`/product/${slug}`}>
                <h3 className="text-sm font-bold text-[#171717] line-clamp-1 font-sans group-hover:text-[#E6321C] transition-colors">
                  {title}
                </h3>
              </Link>
            </div>

            <div className="mt-4 pt-3 border-t border-[#DDD3C5]/60 flex items-center justify-between">
              <div className="flex items-baseline gap-2">
                <span className="text-sm font-bold text-[#171717] font-sans">₹{basePrice}</span>
                {compareAtPrice && compareAtPrice > basePrice && (
